...
class FirstFitBestFitDisasterAware(RoutingBase):
    """FirstFit best fit with disaster node avoidance."""
    ...
    def __rotear_requisicao(
        ...
        ) = FirstFitBestFitDisasterAware.__retorna_informacoes_datapaths(
            requisicao, topology
        )
        if pelo_menos_uma_janela_habil and indice_melhor_caminho is not None:
            FirstFitBestFitDisasterAware.__aloca_requisicao(
                requisicao,
                topology,
                informacoes_dos_datapaths,
                env,
    ...
    def __aloca_requisicao(
        ...
        FirstFitBestFitDisasterAware.__aloca_datapath(
    ...
    def __retorna_informacoes_datapaths(
        ...
        for i, informacoes_caminho in enumerate(caminhos):
            ...
            numero_slots_necessarios = (
                FirstFitBestFitDisasterAware.__slots_nescessarios(
            ...
            ) = FirstFitBestFitDisasterAware.informacoes_sobre_slots(
                caminho, topology, numero_slots_necessarios
            )
            ###slots_livres, slots_livres_agrupados, lista_de_inicios_e_fins = FirstFitBestFitDisasterAware.__informacoes_sobre_slots(caminho, topology)
            dados_do_caminho = {
                "caminho": caminho,
                "distancia": distancia,
    ...
    def informacoes_sobre_slots(
        ...
        for i in range(topology.numero_de_slots):
            if FirstFitBestFitDisasterAware.__checa_concurrency_slot(
                caminho, topology, i
            ):
