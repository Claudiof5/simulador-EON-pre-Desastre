...
class Roteamento_evitando_nodes_pre_desastre(IRoteamento):
    ...
    def rotear_requisicao(
        ...
        requisicao_roteada_com_sucesso = (
            Roteamento_evitando_nodes_pre_desastre.__rotear_requisicao(
    ...
    def __rotear_requisicao(
        ...
        informacoes_dos_datapaths, pelo_menos_uma_janela_habil = (
            Roteamento_evitando_nodes_pre_desastre.__retorna_informacoes_datapaths(
                requisicao, topology
            )
        )
        if pelo_menos_uma_janela_habil:
            Roteamento_evitando_nodes_pre_desastre.__aloca_requisicao(
    ...
    def __aloca_requisicao(
        ...
        for informacoes_datapath in informacoes_datapaths:
            if (
                ...
                Roteamento_evitando_nodes_pre_desastre.__aloca_datapath(
    ...
    def __retorna_informacoes_datapaths(
        ...
        if (
            requisicao.dst
            in topology.caminhos_mais_curtos_entre_links_durante_desastre.keys()
            and requisicao.src
            in topology.caminhos_mais_curtos_entre_links_durante_desastre.keys()
        ):
        ...
        for informacoes_caminho in caminhos:
            ...
            numero_slots_necessarios = (
                Roteamento_evitando_nodes_pre_desastre.__slots_nescessarios(
                    requisicao.bandwidth, fator_de_modulacao
                )
            )

            lista_de_inicios_e_fins, maior_janela_caminho = (
                Roteamento_evitando_nodes_pre_desastre.informacoes_sobre_slots(
                    caminho, topology
                )
            )
    ...
    def informacoes_sobre_slots(
        ...
        for i in range(topology.numero_de_slots):
            if Roteamento_evitando_nodes_pre_desastre.__checa_concurrency_slot(
                caminho, topology, i
            ):
