...
class Roteamento_evitando_nodes_pre_desastre(iRoteamento):
    ...
    def __rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment) -> bool:
        ...
        informacoes_dos_datapaths, pelo_menos_uma_janela_habil = Roteamento_evitando_nodes_pre_desastre.__retorna_informacoes_datapaths(requisicao, topology)
        if pelo_menos_uma_janela_habil:
            Roteamento_evitando_nodes_pre_desastre.__aloca_requisicao(requisicao, topology, informacoes_dos_datapaths, env)
            return True
    ...
    def __aloca_requisicao( requisicao: Requisicao, topology: 'Topologia', informacoes_datapaths: dict, env: Environment) -> None:
        ...
        for informacoes_datapath in informacoes_datapaths:
            if informacoes_datapath["maior_janela_contigua_continua"] >= informacoes_datapath["numero_slots_necessarios"]:
                Roteamento_evitando_nodes_pre_desastre.__aloca_datapath(requisicao, topology, informacoes_datapath, env)
                break
    ...
    def __retorna_informacoes_datapaths( requisicao: Requisicao, topology: 'Topologia') -> tuple[list[dict], bool]:
        ...
        for informacoes_caminho in caminhos:
            ...
            numero_slots_necessarios = Roteamento_evitando_nodes_pre_desastre.__slots_nescessarios(requisicao.bandwidth, fator_de_modulacao)

            lista_de_inicios_e_fins, maior_janela_caminho = Roteamento_evitando_nodes_pre_desastre.informacoes_sobre_slots(caminho, topology)
    ...
    def informacoes_sobre_slots( caminho, topology: 'Topologia') -> tuple[list[tuple[int, int]], int]:
        ...
        for i in range(topology.numero_de_slots):
            if Roteamento_evitando_nodes_pre_desastre.__checa_concurrency_slot(caminho, topology, i):
