...
class RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio(iRoteamento):
    ...
    def __bloqueio_artificial(requisicao: Requisicao, env: Environment) -> bool:
        is_extra_component_traffic = (requisicao.src in COMPONENTE_1 and requisicao.dst in COMPONENTE_2) or \
                                   (requisicao.src in COMPONENTE_2 and requisicao.dst in COMPONENTE_1)
        if not(is_extra_component_traffic):
            return False
            
        probabilidade_de_bloqueio = Registrador.get_last_registro_bloqueio_artificial_por_node(requisicao.src)
    ...
    def rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment ) -> bool:
        ...
        bloqueio_artificial = RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio.__bloqueio_artificial( requisicao, env)
        if bloqueio_artificial:
            return bloqueio_artificial
        
        requisicao_roteada_com_sucesso = RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio.__rotear_requisicao(requisicao, topology, env)
    ...
    def __rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment) -> bool:
        informacoes_dos_datapaths, pelo_menos_uma_janela_habil, indice_melhor_caminho = RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio.__retorna_informacoes_datapaths(requisicao, topology)
        if pelo_menos_uma_janela_habil:
            RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio.__aloca_requisicao(requisicao, topology, informacoes_dos_datapaths, env, indice_melhor_caminho)
            return True
        else:
            requisicao.bloqueia_requisicao( env.now)
            return False
        
    def __aloca_requisicao( requisicao: Requisicao, topology: 'Topologia', informacoes_datapaths: dict, env: Environment, indice_melhor_caminho:int) -> None:
        informacoes_datapath = informacoes_datapaths[indice_melhor_caminho]
        RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio.__aloca_datapath(requisicao, topology, informacoes_datapath, env)
    ...
    def __aloca_datapath( requisicao: Requisicao, topology: 'Topologia', informacoes_datapath: dict, env: Environment) -> None:
        ...
        requisicao.processo_de_desalocacao = env.process(topology.desaloca_janela(caminho, [index_inicio, index_fim], requisicao.holding_time, env))
        requisicao.aceita_requisicao(informacoes_datapath["numero_slots_necessarios"], caminho, len(caminho), [index_inicio, index_fim],
    ...
    def __retorna_informacoes_datapaths( requisicao: Requisicao, topology: 'Topologia') -> tuple[list[dict], bool]:
        ...
        for i, informacoes_caminho in enumerate(caminhos):
            ...
            if not topology.caminho_em_funcionamento(caminho) or not topology.pode_passar_pelo_caminho_que_vai_falhar(caminho):
            ...
            numero_slots_necessarios = RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio.__slots_nescessarios(requisicao.bandwidth, fator_de_modulacao)

            lista_de_inicios_e_fins, tamanho_menor_janela_habil, inicio_menor_janela_habil = RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio.informacoes_sobre_slots(caminho, topology, numero_slots_necessarios)
            ...
            dados_do_caminho = {"caminho": caminho, "distancia": distancia, "fator_de_modulacao": fator_de_modulacao,
                                 ...
                                 "tamanho_menor_janela_habil":tamanho_menor_janela_habil, "inicio_melhor_janela_habil": inicio_menor_janela_habil,
                                  }
            
            lista_de_informacoes_datapath.append(dados_do_caminho)

            if tamanho_menor_janela_habil != None:
                ...
                if tamanho_menor_janela_habil == numero_slots_necessarios:
                    ...
                    break
                elif indice_melhor_caminho == None or tamanho_menor_janela_habil < tamanho_melhor_janela:
                    tamanho_melhor_janela = tamanho_menor_janela_habil
                    indice_melhor_caminho = i - numero_de_caminhos_pulados

        return lista_de_informacoes_datapath, pelo_menos_uma_janela_habil, indice_melhor_caminho
    
    def informacoes_sobre_slots( caminho, topology: 'Topologia', numero_slots_nescessarios: int) -> tuple[list[tuple[int, int]], int]:
        ...
        for i in range(topology.numero_de_slots):
            if RoteamentoBestFit_evitando_nodes_pre_desastre_com_bloqueio.__checa_concurrency_slot(caminho, topology, i):
                ...
                if last_slot_was_free:
                    ...
                    if tamanho_janela_atual >= numero_slots_nescessarios and (tamanho_menor_janela_habil == None or tamanho_janela_atual < tamanho_menor_janela_habil):
        ...
        return lista_de_inicios_e_fins, tamanho_menor_janela_habil, inicio_menor_janela_habil
