...
class Roteamento_evitando_nodes_pre_desastre_com_bloqueio(iRoteamento):
    ...
    def rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment ) -> bool:
        ...
        bloqueio_artificial = Roteamento_evitando_nodes_pre_desastre_com_bloqueio.__bloqueio_artificial( requisicao, env)
        if bloqueio_artificial:
            return bloqueio_artificial
        
        requisicao_roteada_com_sucesso = Roteamento_evitando_nodes_pre_desastre_com_bloqueio.__rotear_requisicao(requisicao, topology, env)
    ...
    def __rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment) -> bool:
        ...
        informacoes_dos_datapaths, pelo_menos_uma_janela_habil = Roteamento_evitando_nodes_pre_desastre_com_bloqueio.__retorna_informacoes_datapaths(requisicao, topology)
        if pelo_menos_uma_janela_habil:
            Roteamento_evitando_nodes_pre_desastre_com_bloqueio.__aloca_requisicao(requisicao, topology, informacoes_dos_datapaths, env)
            return True
    ...
    def __aloca_requisicao( requisicao: Requisicao, topology: 'Topologia', informacoes_datapaths: dict, env: Environment) -> None:
        ...
        for informacoes_datapath in informacoes_datapaths:
            if informacoes_datapath["maior_janela_contigua_continua"] >= informacoes_datapath["numero_slots_necessarios"]:
                Roteamento_evitando_nodes_pre_desastre_com_bloqueio.__aloca_datapath(requisicao, topology, informacoes_datapath, env)
                break
    ...
    def __retorna_informacoes_datapaths( requisicao: Requisicao, topology: 'Topologia') -> tuple[list[dict], bool]:
        ...
        for informacoes_caminho in caminhos:
            ...
            if ( not topology.caminho_em_funcionamento(caminho) or 
            not topology.pode_passar_pelo_caminho_que_vai_falhar(caminho) ):
                continue
            distancia = informacoes_caminho["distancia"]
            fator_de_modulacao = informacoes_caminho["fator_de_modulacao"]
            numero_slots_necessarios = Roteamento_evitando_nodes_pre_desastre_com_bloqueio.__slots_nescessarios(requisicao.bandwidth, fator_de_modulacao)

            lista_de_inicios_e_fins, maior_janela_caminho = Roteamento_evitando_nodes_pre_desastre_com_bloqueio.informacoes_sobre_slots(caminho, topology)
    ...
    def informacoes_sobre_slots( caminho, topology: 'Topologia') -> tuple[list[tuple[int, int]], int]:
        ...
        for i in range(topology.numero_de_slots):
            if Roteamento_evitando_nodes_pre_desastre_com_bloqueio.__checa_concurrency_slot(caminho, topology, i):
