...
class RoteamentoBestFit(iRoteamento):
    ...
    def rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment ) -> bool:
        ...
        requisicao_roteada_com_sucesso = RoteamentoBestFit.__rotear_requisicao(requisicao, topology, env)
    ...
    def __rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment) -> bool:
        informacoes_dos_datapaths, pelo_menos_uma_janela_habil, indice_melhor_caminho = RoteamentoBestFit.__retorna_informacoes_datapaths(requisicao, topology)
        if pelo_menos_uma_janela_habil:
            RoteamentoBestFit.__aloca_requisicao(requisicao, topology, informacoes_dos_datapaths, env, indice_melhor_caminho)
            return True
        else:
    ...
    def __retorna_informacoes_datapaths( requisicao: Requisicao, topology: 'Topologia') -> tuple[list[dict], bool]:
        ...
        for i, informacoes_caminho in enumerate(caminhos):
            ...
            numero_slots_necessarios = RoteamentoBestFit.__slots_nescessarios(requisicao.bandwidth, fator_de_modulacao)

            lista_de_inicios_e_fins, tamanho_menor_janela_habil, inicio_menor_janela_habil = RoteamentoBestFit.informacoes_sobre_slots(caminho, topology, numero_slots_necessarios)
