...
class RoteamentoSubredeEvitandoDesastre(IRoteamento):
    ...
    def rotear_requisicao(
        ...
        """
        requisicao_roteada_com_sucesso = RoteamentoSubredeEvitandoDesastre.__rotear_requisicao(
    ...
    def __rotear_requisicao(
        ...
        informacoes_dos_datapaths, pelo_menos_uma_janela_habil = (
            RoteamentoSubredeEvitandoDesastre.__retorna_informacoes_datapaths(requisicao, topology)
        )
        if pelo_menos_uma_janela_habil:
            RoteamentoSubredeEvitandoDesastre.__aloca_requisicao(
    ...
    def __aloca_requisicao(
        ...
        for informacoes_datapath in informacoes_datapaths:
            if (
                ...
                RoteamentoSubredeEvitandoDesastre.__aloca_datapath(
    ...
    def __retorna_informacoes_datapaths(
        ...
        caminhos_isp_desastre = RoteamentoSubredeEvitandoDesastre._get_caminhos_desastre_from_isp(
            requisicao, topology, isp_requisicao
        )
        
        if caminhos_isp_desastre:
            # Use ISP disaster-aware precomputed paths
            return RoteamentoSubredeEvitandoDesastre._processar_caminhos_isp(
                caminhos_isp_desastre, requisicao, topology
            )
        
        # Fallback to topology-wide disaster paths with ISP filtering
        if (
            int(requisicao.dst) in topology.caminhos_mais_curtos_entre_links_durante_desastre
            and int(requisicao.src) in topology.caminhos_mais_curtos_entre_links_durante_desastre
            and int(requisicao.dst) in topology.caminhos_mais_curtos_entre_links_durante_desastre[int(requisicao.src)]
        ):
            ...
            caminhos_isp_regular = RoteamentoSubredeEvitandoDesastre._get_caminhos_from_isp(
                requisicao, topology, isp_requisicao
            )
        ...
        return RoteamentoSubredeEvitandoDesastre._processar_caminhos_topology(
    ...
    def _processar_caminhos_isp(
        caminhos_isp: list[dict], requisicao: Requisicao, topology: "Topologia"
    ) -> tuple[list[dict], bool]:
        ...
        for informacoes_caminho in caminhos_isp:
            ...
            numero_slots_necessarios = RoteamentoSubredeEvitandoDesastre.__slots_nescessarios(
                requisicao.bandwidth, fator_de_modulacao
            )

            lista_de_inicios_e_fins, maior_janela_caminho = (
                RoteamentoSubredeEvitandoDesastre.informacoes_sobre_slots(caminho, topology)
            )
    ...
    def _processar_caminhos_topology(
        caminhos: list[dict], requisicao: Requisicao, topology: "Topologia", isp_id: int
    ) -> tuple[list[dict], bool]:
        """Process topology-wide paths with ISP filtering.
        ...
        for informacoes_caminho in caminhos:
            ...
            if not RoteamentoSubredeEvitandoDesastre.__caminho_pertence_a_isp(
    ...
    def informacoes_sobre_slots(
        ...
        for i in range(topology.numero_de_slots):
            if RoteamentoSubredeEvitandoDesastre.__checa_concurrency_slot(caminho, topology, i):
