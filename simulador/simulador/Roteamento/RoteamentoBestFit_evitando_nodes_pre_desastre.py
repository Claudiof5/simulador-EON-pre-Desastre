...
class RoteamentoBestFit_evitando_nodes_pre_desastre(iRoteamento):
    ...
    def rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment ) -> bool:
        ...
        requisicao_roteada_com_sucesso = RoteamentoBestFit_evitando_nodes_pre_desastre.__rotear_requisicao(requisicao, topology, env)
    ...
    def __rotear_requisicao( requisicao: Requisicao, topology: 'Topologia', env: Environment) -> bool:
        informacoes_dos_datapaths, pelo_menos_uma_janela_habil, indice_melhor_caminho = RoteamentoBestFit_evitando_nodes_pre_desastre.__retorna_informacoes_datapaths(requisicao, topology)
        if pelo_menos_uma_janela_habil:
            RoteamentoBestFit_evitando_nodes_pre_desastre.__aloca_requisicao(requisicao, topology, informacoes_dos_datapaths, env, indice_melhor_caminho)
            return True
        else:
    ...
    def __aloca_requisicao( requisicao: Requisicao, topology: 'Topologia', informacoes_datapaths: dict, env: Environment, indice_melhor_caminho:int) -> None:
        ...
        RoteamentoBestFit_evitando_nodes_pre_desastre.__aloca_datapath(requisicao, topology, informacoes_datapath, env)
    ...
    def __retorna_informacoes_datapaths( requisicao: Requisicao, topology: 'Topologia') -> tuple[list[dict], bool]:
        ...
        for i, informacoes_caminho in enumerate(caminhos):
            ...
            numero_slots_necessarios = RoteamentoBestFit_evitando_nodes_pre_desastre.__slots_nescessarios(requisicao.bandwidth, fator_de_modulacao)

            lista_de_inicios_e_fins, tamanho_menor_janela_habil, inicio_menor_janela_habil = RoteamentoBestFit_evitando_nodes_pre_desastre.informacoes_sobre_slots(caminho, topology, numero_slots_necessarios)
            ###slots_livres, slots_livres_agrupados, lista_de_inicios_e_fins = RoteamentoBestFit_evitando_nodes_pre_desastre.__informacoes_sobre_slots(caminho, topology)
    ...
    def informacoes_sobre_slots( caminho, topology: 'Topologia', numero_slots_nescessarios: int) -> tuple[list[tuple[int, int]], int]:
        ...
        for i in range(topology.numero_de_slots):
            if RoteamentoBestFit_evitando_nodes_pre_desastre.__checa_concurrency_slot(caminho, topology, i):
