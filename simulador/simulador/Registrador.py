...
Contains the Registrador singleton class that manages statistics during network
...
class Registrador:
    ...
    instance: "Registrador | None" = None

    def __init__(self) -> None:
        """Initialize the Registrador with default metrics and counters.
        ...
        self.current_bloqueio_artificial: dict[int, int] = {
            i: 0 for i in range(1, 25) if i != WANTED_DISASTER_NODE
        }
    ...
    def get_intance() -> "Registrador":
        """Get the singleton instance of Registrador.

        Creates a new instance if none exists, otherwise returns the existing one.

        Returns:
            Registrador: The singleton instance of the Registrador class.
        """
        if Registrador.instance is None:
    ...
    def adiciona_registro_janela_deslizante(
        registro: dict[tuple[int, int], tuple[int, int]],
    ) -> None:
        """Add a sliding window registry entry.

        Args:
            registro (dict[tuple[int, int], (int, int)]): Registry data mapping
                node pairs to (accepted, blocked) counts.
        """
        registrador: Registrador = Registrador.get_intance()
    ...
    def reseta_registrador() -> None:
        ...
        Creates a new fresh instance of Registrador, effectively
    ...
    def get_requisicoes() -> list[Requisicao]:
        ...
        Returns:
            list[Requisicao]: List of all registered request objects.
        """
        registrador: Registrador = Registrador.get_intance()
    ...
    def registra_requisicao_afetada(req: Requisicao) -> None:
        ...
        Args:
            req (Requisicao): The affected request object.
        """
        registrador: Registrador = Registrador.get_intance()
    ...
    def adiciona_requisicao(requisicao: int) -> None:
        ...
        Args:
            requisicao (int): The request object to add to the list.
        """
        registrador: Registrador = Registrador.get_intance()
    ...
    def atualiza_taxa_de_bloqueio_por_par_de_nodes(
        req: Requisicao, env: simpy.Environment, blocked: bool = False
    ) -> Generator:
        ...
        Args:
            req (Requisicao): The request being processed.
            env (simpy.Environment): The simulation environment.
    ...
    def incrementa_numero_requisicoes_aceitas(
        req: Requisicao, env: simpy.Environment
    ) -> None:
        """Increment the count of accepted requests.

        Args:
            req (Requisicao): The accepted request.
            env (simpy.Environment): The simulation environment.
        """
        registrador: Registrador = Registrador.get_intance()
    ...
    def incrementa_numero_requisicoes_bloqueadas(
        ...
        Args:
            req (Requisicao): The blocked request.
            env (simpy.Environment): The simulation environment.
        """
        registrador: Registrador = Registrador.get_intance()
        ...
        env.process(
            Registrador.atualiza_taxa_de_bloqueio_por_par_de_nodes(req, env, True)
        )
    ...
    def incrementa_numero_requisicoes_reroteadas_aceitas(
        ...
        Args:
            req (Requisicao): The rerouted request.
            env (simpy.Environment): The simulation environment.
        """
        registrador: Registrador = Registrador.get_intance()
    ...
    def incrementa_numero_requisicoes_reroteadas_bloqueadas(
        ...
        Args:
            req (Requisicao): The blocked rerouted request.
            env (simpy.Environment): The simulation environment.
        """
        registrador: Registrador = Registrador.get_intance()
    ...
    def salva_resutados(caminho: str) -> None:
        ...
        Serializes the entire Registrador state to JSON format.
