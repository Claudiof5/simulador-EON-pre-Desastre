...
class Desastre:
    ...
    def __ativa_evento(self, informacoes_evento, simulador: "Simulador") -> None:
        if informacoes_evento["tipo"] == "node":
            ...
            Logger.mensagem_acompanha_node_desastre(
                informacoes_evento["node"], simulador.env.now
            )
            ...
            Logger.mensagem_acompanha_link_desastre(
                informacoes_evento["src"], informacoes_evento["dst"], simulador.env.now
            )

    def __desativa_evento(
        self, informacoes_evento: dict, simulador: "Simulador"
    ) -> Generator:
        yield simulador.env.timeout(self.start + self.duration - simulador.env.now)
    ...
    def __falha_no_link(self, node1: int, node2: int, simulador: "Simulador") -> None:
        ...
        if (
            ...
            requisicoes_falhas: list[Requisicao] = self.__quem_falhou_link(
            ...
            for requisicao in requisicoes_falhas:
                if not requisicao.afetada_por_desastre:
                    ...
                    roteador: IRoteamento = simulador.lista_de_isps[
                        index_isp
                    ].roteamento_atual
                    roteador.rerotear_requisicao(requisicao, topology, simulador.env)
                    requisicao.afetada_por_desastre = True

    def __falha_no_no(self, node: int, simulador: "Simulador") -> None:
    ...
    def __quem_falhou_link(
        self, pontaa: int, pontab: int, simulador: "Simulador"
    ) -> list[Requisicao]:
        ...
        for req in requisicoes:
            if (
                ...
                req.dados_pre_reroteamento = (
                    req.retorna_tupla_chave_dicionario_dos_atributos()[1]
                )
